...
router.post('/reset-goals', protect, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const updatedMembers = await User.find({
      _id: { $in: memberIds },
      group: groupName
    }, 'userName goal');
...
router.post('/delete-runs', protect, async (req: AuthenticatedRequest, res) => {
  try {
    const coach = assertUser(req);
    ...
    const group = await Group.findOne({ name: groupName, coach: coach._id });
    if (!group) {
    ...
    const deleteResult = await Run.deleteMany({
      user: { $in: memberIds }
    });
    ...
    const progressResetResult = await User.updateMany(
      { _id: { $in: memberIds } },
      { $set: { progress: 0 } }
    );
...
router.post('/remove-members', protect, async (req: AuthenticatedRequest, res) => {
  try {
    const coach = assertUser(req);
    const { memberIds, groupName } = req.body;
    ...
    group.members = group.members.filter(memberId => 
      !memberObjectIds.includes(memberId.toString())
    );
...
router.get('/my-groups', protect, async (req: AuthenticatedRequest, res) => {
  try {
    const coach = assertUser(req);
...
router.post('/approve-member', protect, async (req: AuthenticatedRequest, res) => {
  try {
    const coach = assertUser(req);
